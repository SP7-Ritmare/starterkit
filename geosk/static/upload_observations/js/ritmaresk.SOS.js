...
if (typeof jQuery === 'undefined') { throw new Error('ritmaresk.SOS\'s JavaScript requires jQuery') }
...
ritmaresk.Sos=( function () {	//sk sos endpoint url example = http://sk.sp7.irea.cnr.it/observations/sos
	var Sos=function (endpointUrl){
		...
		// INIT SOS OBJECT
			...
			self.kvp={
				...
				urlGetResultTemplate: function(offeringId,observedPropertyId){
					return( composeGet({"request":"GetResultTemplate","offering":offeringId,"observedProperty":observedPropertyId,"version":"2.0.0"})	);
				},
				urlDescribeSensor:function(procedureId){
					...
					return( composeGet({"request":"DescribeSensor","procedure":procedureId,"procedureDescriptionFormat":"http://www.opengis.net/sensorML/1.0.1","version":"2.0.0"}) );
				},
				...
				urlGetFeatureOfInterestByProcedure:function(procedureId){
					var postData={"service":"SOS","version":"2.0.0","request":"GetFeatureOfInterest","procedure":procedureId};
				...
				describeSensor:function(procedureId,fx_loaded){
					$.get(
						...
						function(xml){
							...
							if(fx_loaded && (typeof fx_loaded=="function")){fx_loaded(xml);}
						}
					);
				},
			...
			self.json= {
				...
				method:"post",
				describeSensor:function(procedureId,fx_loaded){
					var postData={"service":"SOS","version":"2.0.0","request":"DescribeSensor","procedure":procedureId,"procedureDescriptionFormat": "http://www.opengis.net/sensorML/1.0.1"};
					$.post(
						endpointJson,JSON.stringify(postData),
						function(json){
							//debug(JSON.stringify(json));
							setSensor(json,procedureId);
							if(fx_loaded && (typeof fx_loaded=="function")){fx_loaded(json);}
						}
					);
				},
				getFeatureOfInterestByProcedure:function(procedureId,fx_loaded){
			...
			self.pox = {
				...
				insertResultTemplate:function(proposedTemplate,callback){
					...
					self.pox.postData(proposedTemplate,callback,true);
				...
				insertResult:function(resultTemplateId, resultValues, callback){
					...
					self.pox.postData(xmlStringInsertResult,callback,true);
				...
				postData:function(data,callback,refreshCapabilities){
					$.ajax({
						...
						success:function(xml){	
							debug(xml);						
							if(callback && (typeof callback=="function")){callback(xml);}
							if(refreshCapabilities){self.GetCapabilities();}
						},
		...
		*/
		...
		/*	performs a "sos:getCapabilities" request on the SOS endpoint. Persists the obtained capabilities in the .capabilities variable
		 *	
		 */
		self.GetCapabilities=function(fx_loadedCapabilities){
			...
			*/
			...
			$.post(
				endpointJson, JSON.stringify(postData), 
				function(json){
					setCapabilities(json); // nota bene: ottengo direttamente un oggetto, non devo ulteriormente parsarlo! //JSON.parse(s);
					//if(console){console.debug("capabilities loaded");}
					
					if(fx_loadedCapabilities && (typeof fx_loadedCapabilities=="function")){fx_loadedCapabilities(json);}
				}
			);
		};
		...
		self.getSensorDescription=function(procedure,callback){
			...
			if(description!=null){
				if(callback && (typeof callback=="function")){callback(description);}
			}
			else{
