...
def fixtures(ctx):
    ...
    ctx.run(f"python manage.py loaddata /tmp/sites_docker.json --settings={_localsettings()}", pty=True)
    ctx.run(f"python manage.py loaddata /usr/src/app/fixtures/initial_data.json --settings={_localsettings()}", pty=True)
    ctx.run(f"python manage.py loaddata /tmp/mdtools_services_metadata_docker.json --settings={_localsettings()}", pty=True)
    ctx.run(f"python manage.py loaddata /tmp/apikey_docker.json --settings={_localsettings()}", pty=True)
...
def _prepare_service_metadata_fixture():
    ...
    print(f"node_abstract is {node_abstract}")
    contact_address = os.getenv("SERVICEPROVIDER_ADDRESS", "")
    print(f"contact_address is {contact_address}")
    contact_email = os.getenv("SERVICEPROVIDER_EMAIL", "")
    print(f"contact_email is {contact_email}")
    contact_url = os.getenv("SERVICEPROVIDER_SITE", "")
    print(f"contact_url is {contact_url}")
    contact_stateprovince = os.getenv("SERVICEPROVIDER_STATEPROVINCE", "")
    print(f"contact_stateprovince is {contact_stateprovince}")
    ...
    print(f"contact_postalcode is {contact_postalcode}")
    contact_phone = os.getenv("SERVICEPROVIDER_PHONE", "")
    print(f"contact_phone is {contact_phone}")
    contact_name = os.getenv("SERVICEPROVIDER_NAME", "")
    print(f"contact_name is {contact_name}")
    node_keywords = os.getenv("SERVICEPROVIDER_NODEKEYWORDS", "")
    print(f"node_keywords is {node_keywords}")
...
def _prepare_site_fixture():
    ...
    print(f"Public domain is {domain}")
    port = _geonode_public_port()
    print(f"Public port is {port}")
    default_fixture = [
        {
            "fields": {
                "domain": domain,
...
def _prepare_apikey_fixture():
    ...
    print(f"Tastypie apikey is {api_key}")
    d = datetime.datetime.now()
    mdext_date = f"{d.isoformat()[:23]}Z"
    default_fixture = [
        {
...
def _geoserver_info_provision(url):
    ...
    print(f"GeoServer service url is {cat.service_url}")
    contact_country = os.getenv("SERVICEPROVIDER_COUNTRY", "")
    print(f"contact_country is {contact_country}")
    provider_url = os.getenv("SERVICEPROVIDER_SITE", "")
    print(f"provider_url is {provider_url}")
    ...
    def get_service_json(service):
        ...
        service_base[service]['name'] = f"{node_name} - {service}"
        service_base[service]['title'] = f"{node_title} - {service}"
...
def _pycsw_info_provision():
    ...
    PYCSW = {
        ...
        'CONFIGURATION': {
            'metadata:main': {
                'identification_title': f"{node_title} - Catalog ",
