...
def post_save_nodeconfiguration(request, instance):
    ...
    def get_service_json(instance, service):
        ...
        #                           }}
        service_base[service]['name'] = f"{instance.node_name} - {service} "
        service_base[service]['title'] = f"{instance.node_title} - {service} "
    ...
    try:
        ...
        pycsw_settings_string = f'PYCSW = {str(PYCSW)}'
...
def get_pycsw_configuration(instance):
    PYCSW = {
        ...
        'CONFIGURATION': {
            'metadata:main': {
                'identification_title': f"{instance.node_title} - Catalog ",
...
def set_sensors_configuration(instance):
    configuration_strings = {
        ...
        'misc.defaultProcedurePrefix': f"http://sp7.irea.cnr.it/sensors/{Site.objects.get_current().domain}/procedure/",
        'misc.defaultObservablePropertyPrefix': f"http://sp7.irea.cnr.it/sensors/{Site.objects.get_current().domain}/observableProperty/",
        'misc.defaultFeaturePrefix': f"http://sp7.irea.cnr.it/sensors/{Site.objects.get_current().domain}/foi/",
    }
