...
class MdExtension(models.Model):
    ...
    def ediml_resource(self):
        return f'{settings.SITEURL[:-1]}{self.resource.get_absolute_url()}'
...
class ServicesMetadata(models.Model):
    ...
    def __unicode__(self):
        return f"{self.node_name} - {self.provider_name} - {self.contact_name}"
...
def mdextension_pre_delete(instance, *args, **kwargs):
    ...
    instance.mdextension = None
    MdExtension.objects.filter(resource_id=instance.id).delete()
